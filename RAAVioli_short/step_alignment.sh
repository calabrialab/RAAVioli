for TAG in ${ASSOBCLIST[@]}; do
  ...
  zcat "${TMPDIR}/bcmuxall/${TAG}.cleanR1LCR2LC_1.fastq.gz" \
  | python3 "${RAAVIOLIDIR}/utils_scripts/fqreverse_extract_pureheader.v3.py" \
      "${TMPDIR}/bcmuxall/${TAG}.cleanR1LCR2LC_1_fusion.exclude.list" \
  | pigz -f -c \
  > "${TMPDIR}/bcmuxall/r1.no12.${TAG}.fastq.gz"

  zcat "${TMPDIR}/bcmuxall/${TAG}.cleanR1LCR2LC_2.fastq.gz" \
  | python3 "${RAAVIOLIDIR}/utils_scripts/fqreverse_extract_pureheader.v3.py" \
      "${TMPDIR}/bcmuxall/${TAG}.cleanR1LCR2LC_1_fusion.exclude.list" \
  | pigz -f -c \
  > "${TMPDIR}/bcmuxall/r2.no12.${TAG}.fastq.gz"
  ...
  | samtools view -F 260 -q "${mapQvec}" -O BAM \
