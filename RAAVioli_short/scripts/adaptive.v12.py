...
def checkNoRepeats(read_name, target_chr, target_start, r1_to_check, suboptimalThreshold):
    """
    Memory-optimized version: r1_to_check[read_name] now contains lightweight dicts:
    {
        "chr": <str>,
        "start": <int>,   # 1-based
        "AS": <int or None>,
    ...
    """
    for aln in r1_to_check.get(read_name, []):
        if aln["chr"] == target_chr and aln["start"] == int(target_start):
            AS = aln["AS"]
            XS = aln["XS"]
            if AS is not None and XS is not None:
                if AS >= XS:  # only if the best hit alignment score is better than suboptimal one
...
chrV_name = args.chr_vector_string
...
dict_r2 = {}
r1_step3 = {}
r2_step3 = {}  # will now store lightweight dicts, not pysam objects
...
for read in r2_samfile.fetch():
    ...
    if read.is_read2:
        # small structure for positional lookup
        if qname not in dict_r2:
            dict_r2[qname] = []
        dict_r2[qname].append([chr_, start_1based])

        # small structure for later R2 mate analysis
        if qname not in r2_step3:
            r2_step3[qname] = []

        r2_step3[qname].append({
            "chr": chr_,
            "start": start_1based,          # 1-based
            "end": read.reference_end,      # reference_end is 0-based end (exclusive), but you used it as-is
        ...
        # R1 used only for checkNoRepeats()
        if qname not in r1_step3:
            r1_step3[qname] = []

        r1_step3[qname].append({
            "chr": chr_,
            "start": start_1based,  # 1-based
        ...
        try:
            if read.has_tag("SA"):
                ...
                for i in range(len(all_alignments)):
                    if all_alignments[i][0] == chrV_name:
                ...
                new_aav_string = ",".join(str(x) for x in [chrV_name, chrV_alignments[elem][1],
                                                           int(chrV_alignments[elem][1]) + int(last_matches) - 1,
                                                           chrV_alignments[elem][2]])
                alignment_on_query = ",".join(str(x) for x in [chrV_name, start, end, chrV_alignments[elem][2]])
                ...
                for i in range(1, other_chrV_aln + 1):
                    ...
                    if abs(aav_gap) <= aav_gap_threshold:
                        ...
                        results['aav_matches'] += chrV_cigars_list[elem][index][0]
                        alignment_on_query += ";" + ",".join(
                            str(x) for x in [chrV_name, start, end, chrV_alignments[elem][2]])
                        if aav_gap < 0:
                ...
                results['alignment_flag'] = read.flag
                results['is_proper_pair'] = read.is_proper_pair
                ...
                for i in range(len(others_alignments)):
                    ...
                    if max_gap >= gap >= min_gap and others_alignments[i][0] in dict_r2_chr_list and checkChrPos(
                        ...
                        if checkNoRepeats(read.query_name, others_alignments[i][0], others_alignments[i][1],
                                          r1_step3, suboptimalThreshold):
                            results['gap'] = gap
                            ...
                            if results['gap'] >= 0:
                                ...
                                loc_x = results['end_aav'] + results['gap']
                                results['from_minusN_to_IS'] = original_sequence[
                                                               results['end_aav'] - 12:results['end_aav']]
                            results['integration'] = ":".join(
                                [str(results[intvar]) for intvar in ['target_chr', 'integration_locus',
                                                                     'target_strand']])
                            alignment_on_query += ";" + ",".join(str(x) for x in [others_alignments[i][0], start,
                                                                                   start + getMatchesFromCigarStringOnQuery(
                                                                                       others_alignments[i][3]),
                                                                                   others_alignments[i][2]])
                            break
                        else:
                ...
                if results['gap'] is not None:
                    ...
                    for aln_v2 in list_reads_v2:
                        if aln_v2["chr"] == r1_integration_chr_v2 and (
                                -3000 < aln_v2["start"] - r1_integration_pos_v2 < 3000):
                            strand_v2 = aln_v2["strand"]
                            target_alignment = [
                                aln_v2["chr"],
                                aln_v2["start"] - 1,  # mimic original use of read_v2.pos (0-based)
                                strand_v2,
                                aln_v2["cigar"],
                                aln_v2["mapq"],
                                0,
                            ]
                            results['cigar_r2'] = aln_v2["cigar"]
                            results['pos_r2'] = aln_v2["start"]       # 1-based
                            results['strand_r2'] = strand_v2
                            results['pos_end_r2'] = aln_v2["end"]
                            results['target_matches_r2'] = getMatchesFromCigarString(aln_v2["cigar"])
                            break
                    ...
                    if read.reference_name == chrV_name:
                        r1_onlyaav_counter += 1
                        if len(others_alignments) == 0:
                            r1_no_target_counter += 1
                ...
                results['aav_alignments_start_end'] = ",".join(
                    [str(results[res_var]) for res_var in ['chr', 'aav_last_start', 'aav_last_end',
                                                           'aav_last_strand']])
                results['input_file'] = file_F4
                if read.reference_name == chrV_name:
                    results['start_aav'] = getStartingMismatch(getListFromCigar(read.cigarstring))[
                        0] if strand == "+" else getClosingMismatch(getListFromCigar(read.cigarstring))[0]
                    alignment_on_query = ",".join(
                        str(x) for x in [chrV_name, results['start_aav'], results['start_aav'] + matches,
                                         results['aav_last_strand']])
                    results['alignment_on_query'] = alignment_on_query
                    list_only_rear.append(results)
...
for r1 in r1_reads:
    ...
    for aln in list_reads:
        if aln["chr"] == r1_integration_chr and (-3000 < aln["start"] - r1_integration_pos < 3000):
            target_alignment = [
                aln["chr"],
                aln["start"] - 1,  # mimic original read.pos (0-based)
                aln["strand"],
                aln["cigar"],
                aln["mapq"],
                0,
            ...
            results['n_aav_aln'] = 0
            results['name'] = r1  # read.query_name == r1 in original code
            results['start_aav'] = 0
            results['aav_aln'] = ";"
            results['end_aav'] = 0
