...
parser.add_argument('-o', '--out_path', help='output path for results')
parser.add_argument('-s', '--sysseq', help='path to expo system sequences file')
args = parser.parse_args()
file = args.input
out_path = args.out_path
system_sequences = args.sysseq


df = pd.read_csv(file, sep="\t")
df['pcr_system'] = df['AddedField1'].apply(lambda x: x.split("-")[-1])
df_systems = pd.read_csv(system_sequences,sep="\t")
df=df.merge(df_systems, left_on="pcr_system", right_on="System")
#df['Expo'] = df['Expo_y']
for expo in df.Expo.unique():
    dfe = df[df['Expo']==expo].copy()
    with open(f'{out_path}/{expo}_expo_reads.fa','w+') as f:
...
for index, row in df_systems.iterrows():
    with open(f'{out_path}/{row["Expo"]}.fa','w+') as f:
        f.write(">"+row['Expo']+"\n")
