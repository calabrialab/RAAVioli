...
parser.add_argument('-c', '--clusterd', help='maximum intra-cluster distance')
parser.add_argument('-m', '--mergecol', help='column used to merge clusters shear sites')

args = parser.parse_args()
file = args.input
path = args.output
outputbn = args.basename
clusterd = args.clusterd
merge_id_col = args.mergecol
...
for index, row in df_comb.iterrows():
   ...
   # dfx = dfx.drop('group',axis=1)
    ...
    if ll.shape[0]>1:
        res = hierarchy.fclusterdata(ll, criterion='distance', metric=pairDistance, method='complete', t=clusterd)
    else:
        res=[1]
...
pcc=df.columns[4:]
"""df = df.sort_values(['chr','integration_locus','strand','GeneName',
                                         'GeneStrand','gap','junction'])"""
df.to_csv(path+f"ShsCount_{outputbn}.CLUSTER{clusterd}.tsv",sep="\t",index=False)
...
df_final.to_csv(path+f"ShsCount_{outputbn}.CLUSTER{clusterd}.cleaned1.tsv",sep="\t",index=False)
...
"""df = df.sort_values(['chr','integration_locus','strand','GeneName',
                                         'GeneStrand','gap','junction'])"""
df.to_csv(path+f"SeqCount_{outputbn}.CLUSTER{clusterd}.tsv",sep="\t",index=False)
df[pcc]=df[pcc].replace(1,np.nan)
df['sum']=df[pcc].sum(axis=1)
df = df[df['sum']!=0].copy()
df = df.drop('sum',axis=1)
...
df_final.to_csv(path+f"SeqCount_{outputbn}.CLUSTER{clusterd}.cleaned1.tsv",sep="\t",index=False)
