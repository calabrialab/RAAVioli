...
if [ ${METHOD} = "slim" ]; then
	...
	trimmomatic SE -phred33 -threads $MAXTHREADS ${R1} "/dev/stdout" CROP:80 | fastq_quality_filter -q $FILTER -p 95 -Q 33 | python3 "${RAAVIOLIDIR}/utils_scripts/fastq_to_fasta.tiget.v3.py"  | fasta2csv | cut -d' ' -f1 > ${OUTDIR}/r1.quality.filtered.list
	printf "\n<`date +'%Y-%m-%d %H:%M:%S'`> [TIGET] Creating R2 quality filtered reads list...\n"
	## remove low quality R2 reads
	trimmomatic SE -phred33 -threads $MAXTHREADS ${R2} "/dev/stdout" CROP:12 | fastq_quality_filter -q $FILTER -p 100 -Q 33 | python3 "${RAAVIOLIDIR}/utils_scripts/fastq_to_fasta.tiget.v3.py"  | fasta2csv | cut -d' ' -f1 > ${OUTDIR}/r2.quality.filtered.list
	...
	zcat ${R1} | python3 /home/carlo/workspace/RAAVIOLI/utils_scripts/fqextract_pureheader.v3.py <(cat ${OUTDIR}/r1r2.quality.filtered.list) | pigz --best -f -c > ${OUTDIR}/QF.${R1_NAME} &
	zcat ${R2} | python3 /home/carlo/workspace/RAAVIOLI/utils_scripts/fqextract_pureheader.v3.py <(cat ${OUTDIR}/r1r2.quality.filtered.list) | pigz --best -f -c > ${OUTDIR}/QF.${R2_NAME} &
	wait
...
if [ ${METHOD} = "lam" ]; then
	...
	zcat ${R1} | python3 /home/carlo/workspace/RAAVIOLI/utils_scripts/fqextract_pureheader.v3.py <(cat ${OUTDIR}/r1.quality.filtered.list) | pigz --best -f -c > ${OUTDIR}/QF.${R1_NAME} &
	zcat ${R2} | python3 /home/carlo/workspace/RAAVIOLI/utils_scripts/fqextract_pureheader.v3.py <(cat ${OUTDIR}/r1.quality.filtered.list) | pigz --best -f -c > ${OUTDIR}/QF.${R2_NAME} &
	wait
