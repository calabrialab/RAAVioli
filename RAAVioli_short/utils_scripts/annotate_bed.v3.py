...
+--------------------------------------------------+
 ...
 Description
  - This program automates the annotation in this way:
  	Preconditions:
  	 1. given a GTF file (-> if missed, just do it yourself: mysql --user=genome --host=genome-mysql.cse.ucsc.edu -A -N -e "select * from ensGene;" mm9 | cut -f2- | genePredToGtf file stdin ensGene.gtf )(or in TIGET alien server: mysql --user=annotation --password=annotation --host=172.25.39.2 -A -N -e "select * from refGene;" ucsc_hg19 | cut -f2- | genePredToGtf file stdin refGene.gtf )( genePredToGtf:: http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/genePredToGtf ) and the input BED
  ...
  - Matrix input in a standard matrix file format
...
usage_example = """
Examples of usage:
 (1) Annotate BED given refGene DB as GTF file: 
    <appname> 
...
parser.add_argument('-b', '--inBed', dest="inBed", help="Input BED file. No default option.", action="store",
                    required=True)
parser.add_argument('-a', '--annotation', dest="annotation",
                    help="Annotation file. Couls be GTF, GFF or BED file format, compatible with BEDtools. No default option.",
                    action="store", required=True)
parser.add_argument('-o', '--outBed', dest="outBed", help="Output file name of the BED file. No default option.",
                    action="store", required=True)
parser.add_argument('--bedtoolsOptions', dest="bedtoolsOptions",
                    help="bedtools closest options. Write a csv string with desired options as you were typing them to run bedtools, without the input and output options (of course!) that is the -a and -b parameters. I.e.: Output file name of the BED file. Default: '-d,-D,b,-t,first'.",
                    action="store", required=False, default="-d,-D,b,-t,first")
parser.add_argument('--matrix', dest="matrix", action="store_true",
                    help="Is the input file directly the matrix file? If yes, activate this option and the aprogram will automatically generate the annotated BED file from the matrix. Default: 'False'; alternative: 'True' or just activate the option.",
                    default=False)
...
def runBedTools(inbed, outbed, annotation, optionstring):
    ...
    # 		sys.exit()
    # 	bedout.close()
    print(' '.join(str(x) for x in whattorun))
...
def rewriteBed(inbed, outbed, delimiter="\t", gene_name_position=7):
    ...
    with open(outbed, 'w', newline='') as outfile:
        bedout = csv.writer(outfile, delimiter=delimiter, quoting=csv.QUOTE_MINIMAL)

        # read input in text mode
        with open(inbed, 'r', newline='') as inf:
...
def createBedFromMatrix(matrixfile, ):
    """
	Invoke AWK to format the standard MATRIX file. If this file format changes, pelase change this function!!!
	"""
    filetoreturn = None
    os.system("awk '{print \"chr\"$1\"\t\"$2\"\t\"$2}' '%s' | tail -n +2 > %s" % (matrixfile, filetoreturn))
    return filetoreturn
...
def main():
    ...
    # if input file is BED (not the matrix file)
    if not args.matrix:
        # run bed tools
        statusok = runBedTools(args.inBed, tmpfile, args.annotation, args.bedtoolsOptions)
    else:
        # run bed tools
        inBed = createBedFromMatrix(args.inBed)
        statusok = runBedTools(inBed, tmpfile, args.annotation, args.bedtoolsOptions)
    # rewrite BED
