...
then
    echo "modifying"
   python3 ${RAAVIOLIDIR}/scripts/0.1.modify_raw_matrix.py -i ${TMPDIR}/iss/${POOLSAID}/${output_fn} -o ${TMPDIR}/iss/${POOLSAID}/${output_fn_mod} -a ${ASSOCIATIONFILE}  -n ${POOL}
else
    echo "modfifying"
    python3 ${RAAVIOLIDIR}/scripts/0.1.modify_raw_matrix.py -i ${TMPDIR}/iss/${POOLSAID}/${output_fn} -o ${TMPDIR}/iss/${POOLSAID}/${output_fn_mod} -a ${ASSOCIATIONFILE}  -n ${POOL} -d ${ITR_DF}
fi
...
annotate_matrix -m ${TMPDIR}/matrix/${POOLSAID}/SeqCount_${OUTPUT_NAME}.CLUSTER20.cleaned1.tsv -g ${ANNOTATIONGTF} -t vispa2 -o ${TMPDIR}/matrix/${POOLSAID}/
annotate_matrix -m ${TMPDIR}/matrix/${POOLSAID}/ShsCount_${OUTPUT_NAME}.CLUSTER20.cleaned1.tsv -g ${ANNOTATIONGTF} -t vispa2 -o ${TMPDIR}/matrix/${POOLSAID}/
