...
path = "FINAL/"
df = pd.read_csv(path+"results.SPARK.withrawread.modif.checked.tsv",sep="\t")
...
r_orig = r[old_df_col]
r_new = r[key_cols]
r_new.to_csv(path+"SeqCount_SPARK.minfvalue30.gapabs120.aggregAF1.tsv",sep="\t",index=False)
...
r_orig = r[old_df_col]
r_new = r[key_cols]
r_new.to_csv(path+"ShsCount_SPARK.minfvalue30.gapabs120.aggregAF1.tsv",sep="\t",index=False)
...
cdf.to_csv(path+"ShsCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.tsv",sep="\t",index=False)
...
cdf_cleaned.to_csv(path+"ShsCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.cleaned1.tsv",sep="\t",index=False)
...
cdf.to_csv(path+"SeqCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.tsv",sep="\t",index=False)
cols_sum = cdf.columns[4:]
cdf['sum'] = cdf[cols_sum].sum(axis=1)
cdf_cleaned = cdf[cdf['sum']>1].copy()
cdf_cleaned = cdf_cleaned.drop('sum',axis=1)
cdf_cleaned.to_csv(path+"SeqCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.cleaned1.tsv",sep="\t",index=False)

"""counter=0
lk=[]
for index,value in results_index.items():
    l=len(value)
    if l>1:
        isp = [int(x.split("_")[1]) for x in value]
        k = "_".join(index.split("_")[0:5])

        if max(isp)-min(isp)>20:
            counter+=1
            print(max(isp) - min(isp), index, isp)"""



#annotate_matrix -m ShsCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.cleaned1.tsv -g /opt/genome/monkey/macFas5/annotation/ucsc.macFas5.xenoRefGeneHG19onNM.gtf -o . -t vispa2
#annotate_matrix -m SeqCount_SPARK.minfvalue30.gapabs120.aggregAF1.window8_20.cleaned1.tsv -g /opt/genome/monkey/macFas5/annotation/ucsc.macFas5.xenoRefGeneHG19onNM.gtf -o . -t vispa2

dfgx = pd.DataFrame({'count': df.groupby(
    ["target_chr", "integration_locus","target_strand", "junction_locus", "gap","input_file","AddedField1", "aav_last_strand", "aav_alignments_start_end","n_aav_aln", "total_matches"
                    ]).size()}).reset_index()
dfgx = pd.DataFrame({'count': dfgx.groupby(
    ["target_chr", "integration_locus","target_strand", "junction_locus", "gap","input_file","AddedField1", "aav_last_strand", "aav_alignments_start_end","n_aav_aln"]).size()}).reset_index()

dfgx = pd.DataFrame({'countX': dfgx.groupby(
    ["target_chr", "integration_locus","target_strand", "junction_locus", "gap","AddedField1", "aav_last_strand", "aav_alignments_start_end","n_aav_aln"]).apply(lambda x: x['input_file'].unique())}).reset_index()
dfgx['npcr'] = dfgx['countX'].apply(lambda x: len(x))

cols_keyx = ["target_chr","integration_locus", "target_strand","gap","aav_alignments_start_end","n_aav_aln","aav_last_strand","junction_locus"]

dfgx['keyx'] = dfgx.apply(lambda x: "_".join([str(x[i]) for i in cols_keyx]),axis=1)
new_results = {}
for k in results_index.keys():
    list_if_k = []
    for keyx in results_index[k]:
        dfsub = dfgx[(dfgx['keyx']==keyx) & (dfgx['AddedField1']==k.split("_")[-1])]
        if dfsub.shape[0]>1:
            print("wtf ", keyx)
        elif dfsub.shape[0]==0:
            print("wtf 0! ", keyx)
        else:
            list_if_k.append(list(dfgx[(dfgx['keyx']==keyx) & (dfgx['AddedField1']==k.split("_")[-1])].iloc[0]['countX']))
    new_results[k]=list_if_k