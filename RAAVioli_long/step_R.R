...
lresult <- parseCigarForChimera(df_alm = mixed_aav_hg19,
                                                       quiet = T, cols_to_include = cols_to_include, 
                                                       min_cigar_alm_width = opt$min_cigar_alm_width)

reads_all_alm_by_cigar_chimera<-lresult$df_query_summary
error_reads <- lresult$error_reads
error_reads_file <- paste0(dest_dir,basename_summary,"_error_reads_id.txt")
cat(error_reads, sep="\n", file = error_reads_file)
