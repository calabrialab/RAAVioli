...
parseCigarForChimera <- function(df_alm, 
  ...
  for (read_id in reads_id) 
  {
    tryCatch(
      expr = {
        ...
        for(ixrst in seq_len(nrow(slice_t)))
        {
          cigar_string <- slice_t[ixrst, "cigar"]
          ...
          ir_t_chr <- slice_t[ixrst,"chr"]
          ir_t_strand <- slice_t[ixrst, "strand"]
          ...
          bind_df <- merge(x = bind_df, y = slice_t[ixrst, c(key_cols, cols_to_include)], by = key_cols, all.x = T)
      ...
      error = function(e){
        error_reads<-c(error_reads,read_id)
        print(read_id)
        print(e)
      }
    )
  ...
  return (list(df_query_summary=df_query_summary,error_reads=error_reads))
}
